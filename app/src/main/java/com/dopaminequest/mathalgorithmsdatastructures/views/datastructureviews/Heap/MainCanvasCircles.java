...
public class MainCanvasCircles extends Object {
    ...
    private final int yDimension = HeapView.dimensions.y;
    private final int xDimension = HeapView.dimensions.x;
    private int P_SIZE = HeapView.dimensions.x/110;
    ...
    public MainCanvasCircles(Activity context)
    {
    ...
    public void draw(Canvas canvas)
    {
        ...
        drawGrid(canvas,bottomRowNumNodes,bottomRowNumNodes);
    ...
    public void init()
    {
        ...
        maxBlockHeight = HeapView.dimensions.y*.5f;
        minBlockHeight = HeapView.dimensions.y*.1f;
        size = 0;
        ...
        generalPaint.setShader(new LinearGradient(
                HeapView.dimensions.x / 2f,
                HeapView.dimensions.y / 2f,
                HeapView.dimensions.x,
                HeapView.dimensions.y,
                Color.rgb(180,255,0),
                Color.rgb(0,255,240),
    ...
    public void createSortedList()
    {
        ...
        for(int i = 0; i < numNodes; i++)
        {
            ...
            nodes[i] = new NodeCircle((int) normalizedCircleDiameter, (int) ( minBlockHeight + blockHeight *(i)), p.x, (int) p.y, i+1, this);
            nodesCopyPreSort[i] = new NodeCircle((int) normalizedCircleDiameter, (int) ( minBlockHeight + blockHeight *(i)), p.x, (int) p.y, i+1, this);
        }
...
//        p.x = blockWidth/2 + index*blockWidth;
        ...
        p.x = (int) ((HeapView.dimensions.x/2f*(float)(1f/(float)numNodesCurrentLevel))  + (w*circleDiameter)*( index - ((int)Math.pow(2, log2(index+1))-1)));//(int)numNodesCurrentLevel/2));// + circleDiameter*(1+ index - (int)Math.pow(2, log2(index+1))));//(int)numNodesCurrentLevel/2);
    ...
    public void setNumNodes(int progress)
    {
        ...
        circleDiameter = HeapView.dimensions.x/(float)bottomRowNumNodes;

        yRootStartValue = HeapView.dimensions.y*.7f;
        yBottomEndValue = HeapView.dimensions.y;
...
//        canvas.drawLine(0,HeapView.dimensions.y/2,HeapView.dimensions.x, HeapView.dimensions.y/2, generalPaint5);
        ...
        {
            float currentX = x*(HeapView.dimensions.x/(float)xDim);
            canvas.drawLine(currentX,0,currentX, HeapView.dimensions.y, generalPaint5);
        }

        for(int y = 1; y <= yDim; y++)
        {
            float currentY = y*(HeapView.dimensions.y/(float)yDim);

            canvas.drawLine(0,currentY, HeapView.dimensions.x, currentY, generalPaint5);
        }
    }
}