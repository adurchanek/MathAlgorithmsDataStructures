...
public class MainCanvas extends Object {
    ...
    private final int yDimension = SelectionSortView.dimensions.y;
    private final int xDimension = SelectionSortView.dimensions.x;
    ...
    private class MoveInfo
    {
        ...
        public MoveInfo(int iVal, Point end, int dir, int nNodesToAnimate, boolean cSelectedNode, boolean moveNode, boolean sorted, boolean complete, boolean min)
        {
            endPoint = end;
            direction = dir;
            indexVal = iVal;
    ...
    private void sort() {
        ...
        while(iSort < numNodes)
        {
            ...
            animationSequence.add(new MoveInfo(nodes[iSort].val,  new Point(convertIndexToPosition(jSort)), 1,1, true, false, false, false, true));
            while(jSort < numNodes)
            {
                isSorted = false;
                ...
                boolean isMin = false;
                if(nodes[jSort].val < nodes[minIndex].val)
                {
                    minIndex = jSort;
                    isMin = true;
                }
                animationSequence.add(new MoveInfo(nodes[jSort].val,  new Point(convertIndexToPosition(jSort)), 1,1, true, false, false, false, isMin));
                jSort++;
            }
            ...
            animationSequence.add(new MoveInfo(nodes[minIndex].val,  new Point(convertIndexToPosition(iSort)), 1,2, true, true, isSorted, false, true));
            animationSequence.add(new MoveInfo(nodes[iSort].val,  new Point(convertIndexToPosition(minIndex)), -1,1,false, true,false, false, false));

            Node temp = nodes[minIndex];
            nodes[minIndex] = nodes[iSort];
            nodes[iSort] = temp;
        ...
        {
            animationSequence.add(new MoveInfo(nodes[i].val,  new Point(convertIndexToPosition(i)), 1,1, true, false, false, true, false));
        }
    }
    ...
    public void init()
    {
        ...
        maxBlockHeight = SelectionSortView.dimensions.y*.5f;
        minBlockHeight = SelectionSortView.dimensions.y*.1f;
        size = 0;
    ...
    public void initNodes() {
        ...
        blockWidth = SelectionSortView.dimensions.x/ numNodes;
    ...
    public void moveNode(Node node, Point start,  Point end, int direction)
    {
        ...
        curveControlPoints.set(1, new Point(end.x + (start.x - end.x)/2, (int) (SelectionSortView.dimensions.y*.5f + direction*(SelectionSortView.dimensions.y*.35f + 3*((float)P_SIZE/ node.getScale().y) *SelectionSortView.dimensions.y*.5f))));
        curveControlPoints.set(2, new Point(end.x + (start.x - end.x)/2, (int) (SelectionSortView.dimensions.y*.5f + direction*(SelectionSortView.dimensions.y*.35f + 3*((float)P_SIZE/ node.getScale().y) *SelectionSortView.dimensions.y*.5f))));
    ...
    {
        Point p = new Point();
        p.y = SelectionSortView.dimensions.y/2;
