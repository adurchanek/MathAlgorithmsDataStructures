...
public class NodeCircle extends Object{
    ...
    NodeCircle(int w, int h, int x, int y, int nNumber, MainCanvasCircles mainCanvasCircles)
    {
        mc = mainCanvasCircles;
    ...
    public void update() {
        ...
        animatingStep++;
        if(HeapView.mc.getProgressBarPercentage() == 0f)
        {
        ...
        if(currentAnimationIndex >= linePoints.length - 1)
        {
            ...
            if(HeapView.mc.numberAnimating > 0)
            {
                HeapView.mc.numberAnimating--;
            }
...
//        canvas.restore();
    ...
    public void drawCompletionAnimation(Canvas canvas) {
        ...
        animationCompleteIncrement += (scale.y/ HeapView.mc.minBlockHeight)*P_SIZE*5;
